...
export const fmtUptime=s=>{if(!s)return'—';const d=Math.floor(s/86400),h=Math.floor(s%86400/3600),m=Math.floor(s%3600/60);return`${d}${t('time.dayShort')} ${h}${t('time.hourShort')} ${m}${t('time.minuteShort')}`;};
export const fmtDate=ts=>ts?new Date(ts).toLocaleString(getLocaleTag()):'—';
export const fmtUsd=v=>Number.isFinite(Number(v))?new Intl.NumberFormat(getLocaleTag(),{style:'currency',currency:'USD',maximumFractionDigits:2}).format(Number(v)):'—';
