...
async function renderDashboard(full=true){
  ...
  $('#view').innerHTML=`
   <div class="stats farm-overview-stats">
     <div class="stat"><span>🖥 ${t('dashboard.servers')} ${helpIcon(t('dashboard.serversHelp'))}</span><b>${s.online}/${s.total}</b></div>
     <div class="stat"><span>⚡ ${t('dashboard.total60s')} ${helpIcon(t('dashboard.total60sHelp'))}</span><b>${fmtHash(s.totalHash)}</b></div>
     <div class="stat"><span>💰 ${t('dashboard.income')} ${helpIcon(t('dashboard.incomeHelp'))}</span><b>${Number.isFinite(e.xmrDay)?t('dashboard.xmrDay',{value:Number(e.xmrDay).toFixed(6)}):'—'}</b><small>${Number.isFinite(e.usdDay)?t('dashboard.usdIncome',{day:fmtUsd(e.usdDay),month:fmtUsd(e.usdMonth)}):t('dashboard.waitMonerod')}</small></div>
     <div class="stat health-score ${Number.isFinite(hs)?(hs>=90?'good':hs>=75?'attention':hs>=50?'warn':'bad'):''}"><span>❤ ${t('dashboard.fleetHealth')} ${helpIcon(t('dashboard.healthHelp'))}</span><b>${Number.isFinite(hs)?hs+'/100':'—'}</b><small>${healthLabel}</small></div>
     <div class="stat"><span>🌡 ${t('dashboard.maxCpu')} ${helpIcon(t('dashboard.maxCpuHelp'))}</span><b>${fmtTemp(s.maxTemp)}</b></div>
     <div class="stat"><span>🪙 XMR / USD ${helpIcon(t('dashboard.xmrUsdHelp'))}</span><b>${fmtUsd(overview.market?.price)}</b><small class="${Number(overview.market?.change24h)>=0?'price-up':'price-down'}">${t('dashboard.market24h',{change:fmtPct(overview.market?.change24h)})}</small></div>
     <div class="stat"><span>🔔 ${t('dashboard.alerts')} ${helpIcon(t('dashboard.alertsHelp'))}</span><b>${overview.alerts?.length||0}</b></div>
   </div>
   <div class="panel hero-chart"><div class="panel-head"><div><h2>${t('dashboard.chartTitle')}</h2><span class="muted small">${t('dashboard.chartSubtitle')}</span></div><button id="refresh-all" class="ghost">${t('dashboard.refresh')}</button></div><canvas id="farm-chart"></canvas></div>
   ...
   <div class="panel"><div class="panel-head"><h2>${t('dashboard.activeAlerts')}</h2></div>${alerts}</div>`;
